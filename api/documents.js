...
const buildPdfHealth = () => ({
  success: true,
  route: 'api/documents.js',
...
const handleSignedUrl = async (req, res) => {
  ...
  if (table === 'invoices' && !UUID_REGEX.test(resourceId)) {
...
const handleUploadPdf = async (req, res) => {
  ...
  if (sourceType === 'invoice' && !UUID_REGEX.test(invoiceId)) {
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET' && action === 'pdf-health') {
    return toSafeJson(res, 200, buildPdfHealth());
  }

  if (req.method === 'POST' && action === 'generate-pdf') {
    return generatePdfHandler(req, res);
  }

  if (req.method === 'GET' && action === 'signed-url') {
    return handleSignedUrl(req, res);
  }

  if (req.method === 'POST' && action === 'upload-pdf') {
