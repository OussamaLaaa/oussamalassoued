...
function DateNav({ date, onChange }: { date: string; onChange: (d: string) => void }) {
  return (
    <div className="flex items-center gap-2 mb-4">
      <button onClick={() => { const d = new Date(date); d.setDate(d.getDate() - 1); onChange(d.toISOString().slice(0, 10)); }} className="text-xs px-2 py-1 rounded border border-[#e5e7eb] text-[#0f172a]">←</button>
      <span className="text-xs font-medium text-[#0f172a] min-w-[120px] text-center">{formatDate(date)}</span>
      <button onClick={() => { const d = new Date(date); d.setDate(d.getDate() + 1); onChange(d.toISOString().slice(0, 10)); }} className="text-xs px-2 py-1 rounded border border-[#e5e7eb] text-[#0f172a]">→</button>
      <button onClick={() => onChange(todayStr())} className="text-xs px-2 py-1 rounded bg-[#0f172a] text-white">Today</button>
    </div>
  );
}
...
function WeekNav({ weekStart, onChange }: { weekStart: string; onChange: (w: string) => void }) {
  return (
    <div className="flex items-center gap-2 mb-4">
      <button onClick={() => onChange(addWeeks(weekStart, -1))} className="text-xs px-2 py-1 rounded border border-[#e5e7eb] text-[#0f172a]">←</button>
      <span className="text-xs font-medium text-[#0f172a] min-w-[140px] text-center">Week of {formatDate(weekStart)}</span>
      <button onClick={() => onChange(addWeeks(weekStart, 1))} className="text-xs px-2 py-1 rounded border border-[#e5e7eb] text-[#0f172a]">→</button>
      <button onClick={() => onChange(WEEK_START(new Date()))} className="text-xs px-2 py-1 rounded bg-[#0f172a] text-white">This Week</button>
    </div>
  );
}
...
function LifeDashboard({ lifeNutritionLogs, lifeFitnessLogs, lifeDeenLogs, lifeFamilyActions, lifeWeeklyReviews, sectionNav, selectedDate, selectedWeekStart }: SectionProps) {
  ...
  const cards = [
    ...
    { label: 'Life Score', value: currentReview?.lifeScore ?? '—', color: 'bg-slate-900' },
  ];
    ...
    <div>
      ...
      {currentReview ? (
        <div className="rounded-lg border border-[#e5e7eb] bg-white p-4 shadow-sm">
          <div className="text-sm font-semibold text-[#0f172a] mb-1">Weekly Review for {formatDate(selectedWeekStart)}</div>
          <div className="text-xs text-[#64748b]">{currentReview.summary || 'No summary'}</div>
        ...
        <div className="rounded-lg border border-[#e5e7eb] bg-[#f8fafc] p-4 text-center">
          <div className="text-xs text-[#64748b]">No weekly review for this week. Switch to Weekly Review tab to create one.</div>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div>
      ...
      <div className="rounded-lg border border-[#e5e7eb] bg-white p-4 shadow-sm mb-4">
        <h3 className="text-sm font-semibold text-[#0f172a] mb-3">{editingId ? 'Edit' : 'Add'} Meal Log</h3>
        <form onSubmit={handleSubmit} className="grid grid-cols-2 md:grid-cols-3 gap-3">
          <div>
            <label className="text-xs text-[#64748b] block mb-1">Meal Type</label>
            <select value={form.mealType} onChange={(e) => setForm((p) => ({ ...p, mealType: e.target.value as LifeMealType }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]">
              {MEAL_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
            </select>
          </div>
          <div>
            <label className="text-xs text-[#64748b] block mb-1">Quality</label>
            <select value={form.qualityRating} onChange={(e) => setForm((p) => ({ ...p, qualityRating: e.target.value as LifeQualityRating }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]">
              <option value="">—</option>
              {QUALITY_RATINGS.map((r) => <option key={r} value={r}>{r}</option>)}
            </select>
          </div>
          <div>
            <label className="text-xs text-[#64748b] block mb-1">Energy Level</label>
            <select value={form.energyLevel} onChange={(e) => setForm((p) => ({ ...p, energyLevel: e.target.value as LifeEnergyLevel }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]">
              <option value="">—</option>
              {ENERGY_LEVELS.map((l) => <option key={l} value={l}>{l}</option>)}
            </select>
          </div>
          <div className="col-span-full">
            <label className="text-xs text-[#64748b] block mb-1">Food Description</label>
            <input value={form.foodDescription} onChange={(e) => setForm((p) => ({ ...p, foodDescription: e.target.value }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]" placeholder="What did you eat?" />
          </div>
          <div className="col-span-full">
            <label className="text-xs text-[#64748b] block mb-1">Notes</label>
            <textarea value={form.notes} onChange={(e) => setForm((p) => ({ ...p, notes: e.target.value }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]" rows={2} />
          </div>
          <div className="col-span-full flex gap-2">
            <button type="submit" disabled={saving} className="text-xs px-3 py-1.5 rounded-md bg-[#0f172a] text-white">{saving ? 'Saving...' : editingId ? 'Update' : 'Add'}</button>
            {editingId ? <button type="button" onClick={() => { setEditingId(null); setForm({ mealType: 'meal', foodDescription: '', qualityRating: '', energyLevel: '', notes: '' }); }} className="text-xs px-3 py-1.5 rounded-md border border-[#e5e7eb] text-[#0f172a]">Cancel</button> : null}
          </div>
        </form>
      </div>
      <div className="space-y-2">
        {dayLogs.length === 0 ? <div className="text-xs text-[#64748b] py-4 text-center">No meals logged for this date.</div> : null}
        {dayLogs.map((log) => (
          <div key={log.id} className="rounded-lg border border-[#e5e7eb] bg-white p-3 shadow-sm flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <span className="text-xs font-semibold text-[#0f172a] capitalize">{log.mealType}</span>
                {log.qualityRating ? <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${log.qualityRating === 'good' ? 'bg-emerald-100 text-emerald-800' : log.qualityRating === 'poor' ? 'bg-rose-100 text-rose-800' : 'bg-amber-100 text-amber-800'}`}>{log.qualityRating}</span> : null}
                {log.energyLevel ? <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-slate-100 text-slate-600">{log.energyLevel}</span> : null}
              </div>
              {log.foodDescription ? <div className="text-xs text-[#0f172a]">{log.foodDescription}</div> : null}
              {log.notes ? <div className="text-xs text-[#64748b] mt-0.5">{log.notes}</div> : null}
            </div>
            <div className="flex gap-1 shrink-0">
              <button onClick={() => startEdit(log)} className="text-[10px] px-2 py-1 rounded border border-[#e5e7eb] text-[#64748b]">Edit</button>
              <button onClick={() => onDeleteLifeNutritionLog(log.id)} className="text-[10px] px-2 py-1 rounded border border-rose-200 text-rose-700">Del</button>
            </div>
          </div>
        ))}
      </div>
...
function FitnessSection({ lifeFitnessLogs, onAddLifeFitnessLog, onUpdateLifeFitnessLog, onDeleteLifeFitnessLog, sectionNav, selectedWeekStart, onSetSelectedWeekStart }: SectionProps) {
  ...
  return (
    <div>
      ...
      <div className="text-xs text-[#64748b] mb-3">Total this week: <strong className="text-[#0f172a]">{totalMinutes} minutes</strong></div>
      <div className="rounded-lg border border-[#e5e7eb] bg-white p-4 shadow-sm mb-4">
        <h3 className="text-sm font-semibold text-[#0f172a] mb-3">{editingId ? 'Edit' : 'Add'} Workout</h3>
        <form onSubmit={handleSubmit} className="grid grid-cols-2 md:grid-cols-3 gap-3">
          <div>
            <label className="text-xs text-[#64748b] block mb-1">Workout Type</label>
            <select value={form.workoutType} onChange={(e) => setForm((p) => ({ ...p, workoutType: e.target.value as LifeWorkoutType }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]">
              {WORKOUT_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
            </select>
          </div>
          <div>
            <label className="text-xs text-[#64748b] block mb-1">Duration (min)</label>
            <input type="number" min="0" value={form.durationMinutes} onChange={(e) => setForm((p) => ({ ...p, durationMinutes: e.target.value }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]" />
          </div>
          <div>
            <label className="text-xs text-[#64748b] block mb-1">Intensity</label>
            <select value={form.intensity} onChange={(e) => setForm((p) => ({ ...p, intensity: e.target.value as LifeIntensity }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]">
              <option value="">—</option>
              {INTENSITIES.map((l) => <option key={l} value={l}>{l}</option>)}
            </select>
          </div>
          <div>
            <label className="text-xs text-[#64748b] block mb-1">Date</label>
            <input type="date" value={form.workoutDate || selectedWeekStart} onChange={(e) => setForm((p) => ({ ...p, workoutDate: e.target.value }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]" />
          </div>
          <div className="col-span-full">
            <label className="text-xs text-[#64748b] block mb-1">Exercises</label>
            <textarea value={form.exercises} onChange={(e) => setForm((p) => ({ ...p, exercises: e.target.value }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]" rows={2} />
          </div>
          <div>
            <label className="text-xs text-[#64748b] block mb-1">Body Notes</label>
            <textarea value={form.bodyNotes} onChange={(e) => setForm((p) => ({ ...p, bodyNotes: e.target.value }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]" rows={2} />
          </div>
          <div>
            <label className="text-xs text-[#64748b] block mb-1">Recovery Notes</label>
            <textarea value={form.recoveryNotes} onChange={(e) => setForm((p) => ({ ...p, recoveryNotes: e.target.value }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]" rows={2} />
          </div>
          <div className="col-span-full">
            <label className="text-xs text-[#64748b] block mb-1">Notes</label>
            <textarea value={form.notes} onChange={(e) => setForm((p) => ({ ...p, notes: e.target.value }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]" rows={2} />
          ...
          <div className="col-span-full flex gap-2">
            <button type="submit" disabled={saving} className="text-xs px-3 py-1.5 rounded-md bg-[#0f172a] text-white">{saving ? 'Saving...' : editingId ? 'Update' : 'Add'}</button>
            {editingId ? <button type="button" onClick={() => { setEditingId(null); setForm({ workoutType: 'general', durationMinutes: '', intensity: '', exercises: '', bodyNotes: '', recoveryNotes: '', notes: '', workoutDate: '' }); }} className="text-xs px-3 py-1.5 rounded-md border border-[#e5e7eb] text-[#0f172a]">Cancel</button> : null}
          </div>
        </form>
      </div>
      <div className="space-y-2">
        {weekLogs.length === 0 ? <div className="text-xs text-[#64748b] py-4 text-center">No workouts logged this week.</div> : null}
        {weekLogs.map((log) => (
          <div key={log.id} className="rounded-lg border border-[#e5e7eb] bg-white p-3 shadow-sm flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <span className="text-xs font-semibold text-[#0f172a] capitalize">{log.workoutType}</span>
                {log.durationMinutes ? <span className="text-[10px] text-[#64748b]">{log.durationMinutes} min</span> : null}
                {log.intensity ? <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${log.intensity === 'high' ? 'bg-rose-100 text-rose-800' : log.intensity === 'medium' ? 'bg-amber-100 text-amber-800' : 'bg-slate-100 text-slate-600'}`}>{log.intensity}</span> : null}
              </div>
              <div className="text-[10px] text-[#64748b]">{formatDate(log.workoutDate)}</div>
              {log.exercises ? <div className="text-xs text-[#0f172a] mt-1">{log.exercises}</div> : null}
              {log.bodyNotes ? <div className="text-xs text-[#64748b] mt-0.5">{log.bodyNotes}</div> : null}
            </div>
            <div className="flex gap-1 shrink-0">
              <button onClick={() => startEdit(log)} className="text-[10px] px-2 py-1 rounded border border-[#e5e7eb] text-[#64748b]">Edit</button>
              <button onClick={() => onDeleteLifeFitnessLog(log.id)} className="text-[10px] px-2 py-1 rounded border border-rose-200 text-rose-700">Del</button>
            </div>
          </div>
        ))}
      </div>
...
function DeenSection({ lifeDeenLogs, onAddLifeDeenLog, onUpdateLifeDeenLog, onDeleteLifeDeenLog, sectionNav, selectedDate, onSetSelectedDate }: SectionProps) {
  ...
  return (
    <div>
      ...
      {onSetSelectedDate ? <DateNav date={selectedDate!} onChange={onSetSelectedDate} /> : null}
      <div className="rounded-lg border border-[#e5e7eb] bg-white p-4 shadow-sm">
        <h3 className="text-sm font-semibold text-[#0f172a] mb-1">{existingLog ? 'Edit' : 'Record'} Deen Log</h3>
        <p className="text-[10px] text-[#64748b] mb-3">Track your daily worship and spiritual growth. This is for personal review only.</p>
        <form onSubmit={handleSubmit} className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {prayerFields.map((pf) => (
            <div key={pf.key}>
              <label className="text-xs text-[#64748b] block mb-1">{pf.label}</label>
              <select value={form[pf.key] || ''} onChange={(e) => setForm((p) => ({ ...p, [pf.key]: e.target.value as LifePrayerStatus || undefined }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]">
                <option value="">—</option>
          ...
          <div>
            <label className="text-xs text-[#64748b] block mb-1">Quran (minutes)</label>
            <input type="number" min="0" value={form.quranMinutes ?? ''} onChange={(e) => setForm((p) => ({ ...p, quranMinutes: e.target.value ? Number(e.target.value) : undefined }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]" />
          </div>
          <div>
            <label className="text-xs text-[#64748b] block mb-1">Dhikr Done</label>
            <select value={form.dhikrDone ? 'yes' : form.dhikrDone === false ? 'no' : ''} onChange={(e) => setForm((p) => ({ ...p, dhikrDone: e.target.value === 'yes' ? true : e.target.value === 'no' ? false : undefined }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]">
              <option value="">—</option>
          ...
          <div>
            <label className="text-xs text-[#64748b] block mb-1">Learning (minutes)</label>
            <input type="number" min="0" value={form.learningMinutes ?? ''} onChange={(e) => setForm((p) => ({ ...p, learningMinutes: e.target.value ? Number(e.target.value) : undefined }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]" />
          </div>
          <div className="col-span-full">
            <label className="text-xs text-[#64748b] block mb-1">Charity / Sadaqah Notes</label>
            <textarea value={form.charityNotes} onChange={(e) => setForm((p) => ({ ...p, charityNotes: e.target.value }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]" rows={2} />
          </div>
          <div className="col-span-full">
            <label className="text-xs text-[#64748b] block mb-1">Reflection</label>
            <textarea value={form.reflection} onChange={(e) => setForm((p) => ({ ...p, reflection: e.target.value }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]" rows={2} />
          </div>
          <div className="col-span-full">
            <label className="text-xs text-[#64748b] block mb-1">Notes</label>
            <textarea value={form.notes} onChange={(e) => setForm((p) => ({ ...p, notes: e.target.value }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]" rows={2} />
          ...
          <div className="col-span-full flex gap-2">
            <button type="submit" disabled={saving} className="text-xs px-3 py-1.5 rounded-md bg-[#0f172a] text-white">{saving ? 'Saving...' : existingLog ? 'Update' : 'Save'}</button>
            {existingLog ? <button type="button" onClick={() => onDeleteLifeDeenLog(existingLog.id)} className="text-xs px-3 py-1.5 rounded-md border border-rose-200 text-rose-700">Delete</button> : null}
          </div>
        </form>
      </div>
    </div>
...
function FamilySection({ lifeFamilyActions, onAddLifeFamilyAction, onUpdateLifeFamilyAction, onDeleteLifeFamilyAction, sectionNav, selectedDate, onSetSelectedDate }: SectionProps) {
  ...
  return (
    <div>
      ...
      {onSetSelectedDate ? <DateNav date={selectedDate!} onChange={onSetSelectedDate} /> : null}
      <div className="rounded-lg border border-[#e5e7eb] bg-white p-4 shadow-sm mb-4">
        <h3 className="text-sm font-semibold text-[#0f172a] mb-3">{editingId ? 'Edit' : 'Add'} Family Action</h3>
        <form onSubmit={handleSubmit} className="grid grid-cols-2 md:grid-cols-3 gap-3">
          <div className="col-span-full">
            <label className="text-xs text-[#64748b] block mb-1">Title *</label>
            <input value={form.title} onChange={(e) => setForm((p) => ({ ...p, title: e.target.value }))} required className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]" />
          </div>
          <div>
            <label className="text-xs text-[#64748b] block mb-1">Type</label>
            <select value={form.type} onChange={(e) => setForm((p) => ({ ...p, type: e.target.value as LifeFamilyActionType }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]">
              {FAMILY_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
            </select>
          </div>
          <div>
            <label className="text-xs text-[#64748b] block mb-1">Status</label>
            <select value={form.status} onChange={(e) => setForm((p) => ({ ...p, status: e.target.value as LifeFamilyActionStatus }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]">
              {FAMILY_STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>
          <div>
            <label className="text-xs text-[#64748b] block mb-1">Priority</label>
            <select value={form.priority} onChange={(e) => setForm((p) => ({ ...p, priority: e.target.value as LifePriority }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]">
              {PRIORITIES.map((p) => <option key={p} value={p}>{p}</option>)}
            </select>
          </div>
          <div>
            <label className="text-xs text-[#64748b] block mb-1">Person Name</label>
            <input value={form.personName} onChange={(e) => setForm((p) => ({ ...p, personName: e.target.value }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]" />
          </div>
          <div className="col-span-full">
            <label className="text-xs text-[#64748b] block mb-1">Description</label>
            <textarea value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]" rows={2} />
          </div>
          <div>
            <label className="text-xs text-[#64748b] block mb-1">Outcome</label>
            <textarea value={form.outcome} onChange={(e) => setForm((p) => ({ ...p, outcome: e.target.value }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]" rows={2} />
          </div>
          <div>
            <label className="text-xs text-[#64748b] block mb-1">Next Action</label>
            <textarea value={form.nextAction} onChange={(e) => setForm((p) => ({ ...p, nextAction: e.target.value }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]" rows={2} />
          </div>
          <div className="col-span-full">
            <label className="text-xs text-[#64748b] block mb-1">Notes</label>
            <textarea value={form.notes} onChange={(e) => setForm((p) => ({ ...p, notes: e.target.value }))} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]" rows={2} />
          ...
          <div className="col-span-full flex gap-2">
            <button type="submit" disabled={saving} className="text-xs px-3 py-1.5 rounded-md bg-[#0f172a] text-white">{saving ? 'Saving...' : editingId ? 'Update' : 'Add'}</button>
            {editingId ? <button type="button" onClick={() => { setEditingId(null); setForm({ title: '', type: 'other', status: 'planned', priority: 'medium', personName: '', description: '', outcome: '', nextAction: '', notes: '' }); }} className="text-xs px-3 py-1.5 rounded-md border border-[#e5e7eb] text-[#0f172a]">Cancel</button> : null}
          </div>
        </form>
      </div>
      <div className="space-y-2">
        {lifeFamilyActions.length === 0 ? <div className="text-xs text-[#64748b] py-4 text-center">No family actions yet.</div> : null}
        {lifeFamilyActions.filter((a) => !selectedDate || a.actionDate === selectedDate).map((action) => (
          <div key={action.id} className="rounded-lg border border-[#e5e7eb] bg-white p-3 shadow-sm flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                <span className="text-xs font-semibold text-[#0f172a]">{action.title}</span>
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${action.type === 'call' ? 'bg-blue-100 text-blue-800' : action.type === 'visit' ? 'bg-green-100 text-green-800' : action.type === 'support' ? 'bg-purple-100 text-purple-800' : 'bg-slate-100 text-slate-600'}`}>{action.type}</span>
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${action.status === 'done' ? 'bg-emerald-100 text-emerald-800' : action.status === 'postponed' ? 'bg-amber-100 text-amber-800' : action.status === 'cancelled' ? 'bg-rose-100 text-rose-800' : 'bg-slate-100 text-slate-600'}`}>{action.status}</span>
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${action.priority === 'high' ? 'bg-rose-50 text-rose-700' : action.priority === 'low' ? 'bg-slate-50 text-slate-500' : 'bg-amber-50 text-amber-700'}`}>{action.priority}</span>
              </div>
              {action.personName ? <div className="text-[10px] text-[#64748b]">{action.personName}</div> : null}
              {action.description ? <div className="text-xs text-[#0f172a] mt-1">{action.description}</div> : null}
              {action.outcome ? <div className="text-xs text-[#64748b] mt-0.5">Outcome: {action.outcome}</div> : null}
            </div>
            <div className="flex gap-1 shrink-0">
              {action.status !== 'done' ? <button onClick={() => markDone(action)} className="text-[10px] px-2 py-1 rounded border border-emerald-200 text-emerald-700">Done</button> : null}
              {action.status !== 'postponed' && action.status !== 'done' ? <button onClick={() => markPostponed(action)} className="text-[10px] px-2 py-1 rounded border border-amber-200 text-amber-700">Postpone</button> : null}
              <button onClick={() => startEdit(action)} className="text-[10px] px-2 py-1 rounded border border-[#e5e7eb] text-[#64748b]">Edit</button>
              <button onClick={() => onDeleteLifeFamilyAction(action.id)} className="text-[10px] px-2 py-1 rounded border border-rose-200 text-rose-700">Del</button>
            </div>
          </div>
        ))}
      </div>
...
function WeeklyReviewSection({ lifeWeeklyReviews, onAddLifeWeeklyReview, onUpdateLifeWeeklyReview, onDeleteLifeWeeklyReview, sectionNav, selectedWeekStart, onSetSelectedWeekStart }: SectionProps) {
  ...
  const inputRow = (label: string, value: string, onChange: (v: string) => void, options?: { rows?: number; placeholder?: string }) => (
    <div className="col-span-full">
      ...
      <textarea value={value} onChange={(e) => onChange(e.target.value)} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]" rows={options?.rows || 2} placeholder={options?.placeholder} />
    </div>
  );
  ...
  const scoreInput = (label: string, value: number | undefined, onChange: (v: number | undefined) => void) => (
    <div>
      <label className="text-xs text-[#64748b] block mb-1">{label}</label>
      <select value={value ?? ''} onChange={(e) => onChange(e.target.value ? Number(e.target.value) : undefined)} className="w-full text-xs px-2 py-1.5 rounded border border-[#e5e7eb]">
        <option value="">—</option>
  ...
  return (
    <div>
      ...
      {onSetSelectedWeekStart ? <WeekNav weekStart={selectedWeekStart!} onChange={onSetSelectedWeekStart} /> : null}
      <div className="rounded-lg border border-[#e5e7eb] bg-white p-4 shadow-sm">
        <h3 className="text-sm font-semibold text-[#0f172a] mb-1">{existingReview ? 'Edit' : 'Create'} Weekly Review</h3>
        <p className="text-[10px] text-[#64748b] mb-3">Review your week across health, nutrition, fitness, deen, and family.</p>
        <form onSubmit={handleSubmit} className="grid grid-cols-2 md:grid-cols-3 gap-3">
          ...
          <div className="col-span-full flex gap-2">
            <button type="submit" disabled={saving} className="text-xs px-3 py-1.5 rounded-md bg-[#0f172a] text-white">{saving ? 'Saving...' : existingReview ? 'Update' : 'Save'}</button>
            {existingReview ? <button type="button" onClick={() => onDeleteLifeWeeklyReview(existingReview.id)} className="text-xs px-3 py-1.5 rounded-md border border-rose-200 text-rose-700">Delete</button> : null}
          </div>
        </form>
      </div>
    </div>
