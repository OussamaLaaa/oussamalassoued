  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <div className="text-xs font-semibold uppercase tracking-[0.18em] text-neutral-500">AI Notes Assistant</div>
          <h3 className="mt-2 text-lg font-semibold text-neutral-900">Organize and improve notes with review-first AI</h3>
          <p className="mt-1 text-sm text-neutral-500">Review before applying.</p>
        </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="space-y-4">
          <div>
            <div className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-500">Mode</div>
              ...
              {MODE_OPTIONS.map((option) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-1 text-xs leading-5 text-neutral-500">{option.description}</div>
                  </button>
                );
              })}
            </div>
          ...
          <label className="block space-y-1.5">
            <span className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-500">Instructions</span>
            <textarea
              value={instructions}
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <label className="block space-y-1.5">
              <span className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-500">Language</span>
            ...
            <div className="flex items-end">
              <button
                ...
                onClick={analyze}
                disabled={loading}
                className="w-full rounded-md bg-neutral-900 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-neutral-800 disabled:cursor-not-allowed disabled:opacity-60"
              >
          ...
          {copiedMessage ? <div className="rounded-md border border-neutral-200 bg-neutral-50 px-3 py-2 text-sm text-neutral-700">{copiedMessage}</div> : null}
        </div>

        <div className="space-y-3">
          <div className="rounded-md border border-neutral-200 bg-neutral-50 p-4">
            <div className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-500">Context</div>
          ...
          <div className="rounded-md border border-neutral-200 bg-neutral-50 p-4">
            <div className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-500">Raw note</div>
            <p className="mt-3 max-h-48 overflow-auto whitespace-pre-wrap text-sm leading-6 text-neutral-900">
      ...
      {result ? (
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="rounded-xl border border-neutral-200 bg-white p-5">
            <div className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-500">Summary</div>
            <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-neutral-900">{result.summary || 'No summary returned.'}</p>
            <div className="mt-4 flex flex-wrap gap-2">
              <button type="button" onClick={handleCopyImprovedContent} disabled={!result.improvedContent} className="rounded-md border border-neutral-200 bg-white px-3 py-2 text-xs font-semibold text-neutral-700 hover:bg-neutral-50 disabled:opacity-50">
                Copy Improved Content
              </button>
              <button type="button" onClick={handleReplaceContent} disabled={!result.improvedContent} className="rounded-md bg-neutral-900 px-3 py-2 text-xs font-semibold text-white hover:bg-neutral-800 disabled:opacity-50">
                Replace Note Content
              </button>
              <button type="button" onClick={handleAppendSummary} disabled={!result.summary && !result.keyPoints.length} className="rounded-md border border-neutral-200 bg-white px-3 py-2 text-xs font-semibold text-neutral-700 hover:bg-neutral-50 disabled:opacity-50">
          ...
          <div className="rounded-xl border border-neutral-200 bg-white p-5">
            <div className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-500">Improved Content</div>
            <p className="mt-3 max-h-48 overflow-auto whitespace-pre-wrap text-sm leading-6 text-neutral-900">
          ...
          <div className="rounded-xl border border-neutral-200 bg-white p-5">
            <div className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-500">Key Points</div>
            <ul className="mt-3 space-y-2 text-sm leading-6 text-neutral-900">
              {result.keyPoints.length ? result.keyPoints.map((point) => <li key={point} className="rounded-md border border-neutral-200 bg-neutral-50 px-3 py-2">{point}</li>) : <li className="text-neutral-500">No key points returned.</li>}
            </ul>
          </div>

          <div className="rounded-xl border border-neutral-200 bg-white p-5">
            <div className="flex items-center justify-between gap-3">
              <div className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-500">Suggested Tasks</div>
              <button type="button" onClick={handleCopyTasks} disabled={!result.suggestedTasks.length} className="rounded-md border border-neutral-200 bg-white px-3 py-2 text-xs font-semibold text-neutral-700 hover:bg-neutral-50 disabled:opacity-50">
                Copy Suggested Tasks
              </button>
            </div>
            <div className="mt-3 space-y-3">
              {result.suggestedTasks.length ? result.suggestedTasks.map((task, index) => (
                <div key={`${task.title}-${index}`} className="rounded-md border border-neutral-200 bg-neutral-50 p-3">
                  <div className="text-sm font-semibold text-neutral-900">{task.title}</div>
                  <div className="mt-1 flex flex-wrap gap-2 text-xs text-neutral-600">
                    {task.priority ? <span className="rounded-full border border-neutral-200 bg-white px-2 py-1">Priority: {task.priority}</span> : null}
                    {task.category ? <span className="rounded-full border border-neutral-200 bg-white px-2 py-1">Category: {task.category}</span> : null}
                    {task.suggestedDueDate ? <span className="rounded-full border border-neutral-200 bg-white px-2 py-1">Due: {task.suggestedDueDate}</span> : null}
                  </div>
                  {task.notes ? <p className="mt-2 text-sm text-neutral-600">{task.notes}</p> : null}
                </div>
              )) : <p className="text-sm text-neutral-500">No suggested tasks returned.</p>}
            </div>
          </div>

          <div className="rounded-xl border border-neutral-200 bg-white p-5">
            <div className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-500">Suggested Category</div>
            <div className="mt-3 text-sm text-neutral-900">{matchingCategory?.name || result.suggestedCategory || 'No category suggested.'}</div>
            <div className="mt-4 flex flex-wrap gap-2">
              <button type="button" onClick={handleApplyCategoryTags} disabled={!result.suggestedCategory && !result.suggestedTags.length} className="rounded-md bg-neutral-900 px-3 py-2 text-xs font-semibold text-white hover:bg-neutral-800 disabled:opacity-50">
          ...
          <div className="rounded-xl border border-neutral-200 bg-white p-5">
            <div className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-500">Suggested Tags</div>
            <div className="mt-3 flex flex-wrap gap-2">
              {result.suggestedTags.length ? result.suggestedTags.map((tag) => (
                <span key={tag} className="rounded-full border border-neutral-200 bg-neutral-50 px-3 py-1 text-xs font-medium text-neutral-700">
                  {tag}
                </span>
              )) : <span className="text-sm text-neutral-500">No tags suggested.</span>}
            </div>
          </div>

          <div className="rounded-xl border border-neutral-200 bg-white p-5 lg:col-span-2">
            <div className="text-xs font-semibold uppercase tracking-[0.14em] text-neutral-500">Next Actions</div>
            <ul className="mt-3 space-y-2 text-sm leading-6 text-neutral-900">
              {result.nextActions.length ? result.nextActions.map((action) => <li key={action} className="rounded-md border border-neutral-200 bg-neutral-50 px-3 py-2">{action}</li>) : <li className="text-neutral-500">No next actions returned.</li>}
            </ul>
          </div>
        </div>
      ) : null}
